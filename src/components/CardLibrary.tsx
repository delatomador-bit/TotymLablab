...
export default function CardLibrary({
  ...
  const filtered = useMemo(() => {
    ...
    return allCards.filter((c) => {
      ...
      if (typeFilter === 'utility' && !isGenericCard(c)) return false;
      if (typeFilter === 'all' && isGenericCard(c)) return false;
      if (c.cardType === 'tarot') {
        if (arcanaFilter !== 'all' && c.arcanaType !== arcanaFilter) return false;
        if (suitFilter !== 'all' && c.suit !== suitFilter) return false;
        if (targetFilter !== 'any' && c.targetType !== targetFilter) return false;
      }
      return true;
    });
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div>
            ...
            <div className="filter-row">
              ...
              <button className="chip" data-active={typeFilter === 'creature'} onClick={() => setTypeFilter('creature')}>Creatures</button>
              <button className="chip" data-active={typeFilter === 'tarot'} onClick={() => setTypeFilter('tarot')}>Tarot</button>
              <button className="chip" data-active={typeFilter === 'utility'} onClick={() => setTypeFilter('utility')}>Deck-count utility</button>
            </div>
          </div>
          ...
          {typeFilter === 'tarot' || typeFilter === 'all' ? (
            <>
              <div>
                ...
                <div className="filter-row">
                  <button className="chip" data-active={arcanaFilter === 'all'} onClick={() => setArcanaFilter('all')}>Any</button>
                  <button className="chip" data-active={arcanaFilter === 'major'} onClick={() => setArcanaFilter('major')}>Major</button>
                  <button className="chip" data-active={arcanaFilter === 'minor'} onClick={() => setArcanaFilter('minor')}>Minor</button>
                </div>
              </div>
              <div>
                <div className="section-label">Suit</div>
                <div className="filter-row">
                  <button className="chip" data-active={suitFilter === 'all'} onClick={() => setSuitFilter('all')}>Any</button>
                  <button className="chip" data-active={suitFilter === 'wands'} onClick={() => setSuitFilter('wands')}>Wands</button>
                  <button className="chip" data-active={suitFilter === 'cups'} onClick={() => setSuitFilter('cups')}>Cups</button>
                  <button className="chip" data-active={suitFilter === 'swords'} onClick={() => setSuitFilter('swords')}>Swords</button>
                  <button className="chip" data-active={suitFilter === 'pentacles'} onClick={() => setSuitFilter('pentacles')}>Pentacles</button>
                </div>
              </div>
              <div>
                <div className="section-label">Target</div>
                <div className="filter-row">
                  <button className="chip" data-active={targetFilter === 'any'} onClick={() => setTargetFilter('any')}>Any</button>
                  <button className="chip" data-active={targetFilter === 'self'} onClick={() => setTargetFilter('self')}>Self</button>
                  <button className="chip" data-active={targetFilter === 'opponent'} onClick={() => setTargetFilter('opponent')}>Opponent</button>
                  <button className="chip" data-active={targetFilter === 'two_opponents'} onClick={() => setTargetFilter('two_opponents')}>Two opp.</button>
                  <button className="chip" data-active={targetFilter === 'all'} onClick={() => setTargetFilter('all')}>All</button>
                </div>
              </div>
            </>
          ) : null}
          ...
          <div className="card-list" style={{ marginTop: 4 }}>
            {filtered.length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--text-dim)', padding: '12px', textAlign: 'center' }}>
                  ...
                  >
                    ...
                    <div className="card-row-meta">
                      ...
                      {card.cardType === 'tarot' && card.arcanaType && (
                        <span>{card.arcanaType === 'major' ? 'Major' : 'Minor'}{card.suit ? ` · ${SUIT_LABELS[card.suit]}` : ''}</span>
                      )}
                      {card.cardType === 'tarot' && card.targetType && (
                        <span>· {TARGET_LABELS[card.targetType]}</span>
                      )}
                      {isGenericCard(card) && <span className="util-flag">— placeholder</span>}
                    </div>
                  </button>
                  <button
          ...
          {card.cardType === 'tarot' && (
            <>
              <div className="detail-row">
                ...
                <span className="detail-val">{card.arcanaType === 'major' ? 'Major' : card.arcanaType === 'minor' ? 'Minor' : '—'}</span>
              </div>
              <div className="detail-row">
                <span className="detail-label">Suit</span>
                <span className="detail-val">{card.suit ? SUIT_LABELS[card.suit] : '—'}</span>
              </div>
              <div className="detail-row">
                <span className="detail-label">Target</span>
                <span className="detail-val">{card.targetType ? TARGET_LABELS[card.targetType] : '—'}</span>
              </div>
            </>
          )}
            ...
            <>
              <div style={{ marginTop: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                  <ReqSide label="Left" req={card.creatureRequirements.left} />
                  <ReqSide label="Right" req={card.creatureRequirements.right} />
                </div>
              </div>
              {card.immunity && (
...
function ReqSide({ label, req }: { label: string; req: { clan: string; required: number } }) {
  ...
  return (
    <div className="req-side">
      ...
      <span>{CLAN_LABELS[clan]} {req.required}</span>
    </div>
  );
}
